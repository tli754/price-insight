...
function makeTasksReadyResponse(items: { id: string; endpoint_advanced: string }[]) {
  return {
...
// Full flow mock: handles all 4 endpoint types using tasks_ready
function mockFullFlow(shoppingItems: object[] = [makeShoppingItem()], sellers: object[] = [makeSeller()]) {
  return vi.spyOn(global, "fetch").mockImplementation(async (input) => {
    ...
    if (url.includes("products/tasks_ready"))
      return new Response(JSON.stringify(makeTasksReadyResponse([{ id: "s-task", endpoint_advanced: "/v3/merchant/google/products/task_get/advanced/s-task" }])), { status: 200 });
    ...
    if (url.includes("product_info/tasks_ready"))
      return new Response(JSON.stringify(makeTasksReadyResponse([{ id: "i-task", endpoint_advanced: "/v3/merchant/google/product_info/task_get/advanced/i-task" }])), { status: 200 });
...
describe("DataForSeoService.getShoppingCandidates()", () => {
  it("returns candidates when task appears in tasks_ready", async () => {
    vi.spyOn(global, "fetch").mockImplementation(async (input) => {
      const url = input.toString();
      if (url.includes("tasks_ready"))
        return new Response(JSON.stringify(makeTasksReadyResponse([{ id: "task-001", endpoint_advanced: "/v3/merchant/google/products/task_get/advanced/task-001" }])), { status: 200 });
      return new Response(JSON.stringify(makeShoppingGetResponse([makeShoppingItem()])), { status: 200 });
    });
  ...
  it("maps rank_absolute to googlePosition", async () => {
    vi.spyOn(global, "fetch").mockImplementation(async (input) => {
      const url = input.toString();
      if (url.includes("tasks_ready"))
        return new Response(JSON.stringify(makeTasksReadyResponse([{ id: "t", endpoint_advanced: "/v3/merchant/google/products/task_get/advanced/t" }])), { status: 200 });
      return new Response(JSON.stringify(makeShoppingGetResponse([makeShoppingItem({ rank_absolute: 5 })])), { status: 200 });
    });
  ...
  it("maps tag from tags[0]", async () => {
    vi.spyOn(global, "fetch").mockImplementation(async (input) => {
      ...
      if (url.includes("tasks_ready"))
        return new Response(JSON.stringify(makeTasksReadyResponse([{ id: "t", endpoint_advanced: "/v3/merchant/google/products/task_get/advanced/t" }])), { status: 200 });
      return new Response(JSON.stringify(makeShoppingGetResponse([makeShoppingItem({ tags: ["SALE"] })])), { status: 200 });
    });
    const [c] = await makeService().getShoppingCandidates("t");
    expect(c.tag).toBe("SALE");
  });

  it("polls tasks_ready until task ID appears", async () => {
    let callCount = 0;
    vi.spyOn(global, "fetch").mockImplementation(async (input) => {
      const url = input.toString();
      if (url.includes("tasks_ready")) {
        callCount++;
        const items = callCount >= 2 ? [{ id: "task-001", endpoint_advanced: "/v3/merchant/google/products/task_get/advanced/task-001" }] : [];
        return new Response(JSON.stringify(makeTasksReadyResponse(items)), { status: 200 });
      }
      return new Response(JSON.stringify(makeShoppingGetResponse([makeShoppingItem()])), { status: 200 });
    });
    vi.useFakeTimers();
  ...
  it("returns empty array when task never appears in tasks_ready", async () => {
    vi.spyOn(global, "fetch").mockImplementation(async () =>
      new Response(JSON.stringify(makeTasksReadyResponse([])), { status: 200 })
    );
    vi.useFakeTimers();
    const promise = makeService().getShoppingCandidates("task-001");
    await vi.runAllTimersAsync();
  ...
  it("skips items with type !== google_shopping_serp", async () => {
    vi.spyOn(global, "fetch").mockImplementation(async (input) => {
      ...
      if (url.includes("tasks_ready"))
        return new Response(JSON.stringify(makeTasksReadyResponse([{ id: "t", endpoint_advanced: "/v3/merchant/google/products/task_get/advanced/t" }])), { status: 200 });
      return new Response(JSON.stringify(makeShoppingGetResponse([
  ...
  it("skips items with null product_id", async () => {
    vi.spyOn(global, "fetch").mockImplementation(async (input) => {
      ...
      if (url.includes("tasks_ready"))
        return new Response(JSON.stringify(makeTasksReadyResponse([{ id: "t", endpoint_advanced: "/v3/merchant/google/products/task_get/advanced/t" }])), { status: 200 });
      return new Response(JSON.stringify(makeShoppingGetResponse([makeShoppingItem({ product_id: null })])), { status: 200 });
    });
    const candidates = await makeService().getShoppingCandidates("t");
    expect(candidates).toHaveLength(0);
  });
  ...
  it("skips items with currency !== NZD", async () => {
    vi.spyOn(global, "fetch").mockImplementation(async (input) => {
      ...
      if (url.includes("tasks_ready"))
        return new Response(JSON.stringify(makeTasksReadyResponse([{ id: "t", endpoint_advanced: "/v3/merchant/google/products/task_get/advanced/t" }])), { status: 200 });
      return new Response(JSON.stringify(makeShoppingGetResponse([makeShoppingItem({ currency: "AUD" })])), { status: 200 });
    });
    const candidates = await makeService().getShoppingCandidates("t");
    expect(candidates).toHaveLength(0);
  });
  ...
  it("caps candidates at 20", async () => {
    const items = Array.from({ length: 25 }, (_, i) => makeShoppingItem({ product_id: `prod-${i}` }));
    vi.spyOn(global, "fetch").mockImplementation(async (input) => {
      const url = input.toString();
      if (url.includes("tasks_ready"))
        return new Response(JSON.stringify(makeTasksReadyResponse([{ id: "t", endpoint_advanced: "/v3/merchant/google/products/task_get/advanced/t" }])), { status: 200 });
      return new Response(JSON.stringify(makeShoppingGetResponse(items)), { status: 200 });
    });
    const candidates = await makeService().getShoppingCandidates("t");
...
describe("DataForSeoService.searchShoppingPrices()", () => {
  ...
  it("returns empty array when Shopping task yields no candidates", async () => {
    vi.spyOn(global, "fetch").mockImplementation(async (input) => {
      ...
      if (url.includes("products/tasks_ready")) return new Response(JSON.stringify(makeTasksReadyResponse([{ id: "s-task", endpoint_advanced: "/v3/merchant/google/products/task_get/advanced/s-task" }])), { status: 200 });
  ...
  it("continues when one Product Info task fails", async () => {
    ...
    vi.spyOn(global, "fetch").mockImplementation(async (input) => {
      ...
      if (url.includes("product_info/tasks_ready")) return new Response(JSON.stringify(makeTasksReadyResponse([{ id: "i-task", endpoint_advanced: "/v3/merchant/google/product_info/task_get/advanced/i-task" }])), { status: 200 });
