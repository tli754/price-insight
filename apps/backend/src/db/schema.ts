...
export const products = pgTable(
  "products",
  {
    ...
    // updated_at is kept current by the set_updated_at() trigger defined in
    // docs/data/price_insight_supabase_schema.sql — Postgres has no
    // ON UPDATE CURRENT_TIMESTAMP column sugar (MySQL's .onUpdateNow()), so
    // repositories must NOT set this column manually on .update() calls.
...
export const priceInsights = pgTable(
  "price_insights",
  {
    id: integer("id").generatedByDefaultAsIdentity().primaryKey(),
    productId: integer("product_id")
...
export const customers = pgTable(
  "customers",
  {
    ...
    createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
    // See products.updatedAt above — kept current by the DB trigger, not app code.
    updatedAt: timestamp("updated_at", { withTimezone: true }).notNull().defaultNow()
  },
  (table) => ({
...
export const orders = pgTable(
  "orders",
  {
    ...
    customerId: integer("customer_id").references(() => customers.id, { onDelete: "set null", onUpdate: "cascade" }),
...
export const orderItems = pgTable(
  "order_items",
  {
    ...
    productId: integer("product_id").references(() => products.id, { onDelete: "set null", onUpdate: "cascade" }),
...
export const productAiReports = pgTable(
  "product_ai_reports",
  {
    id: integer("id").generatedByDefaultAsIdentity().primaryKey(),
    productId: integer("product_id")
    ...
    reportTypes: jsonb("report_types").$type<string[]>().notNull(),
